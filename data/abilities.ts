...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	volcanicash: {
		onFoeTryMove(target, source, move) {
			const blockedMoves = ['trickroom', 'tailwind'];
	
			// Check if the move is in the list of blocked moves
			if (blockedMoves.includes(move.id)) {
				// Prevent the move if it affects either side or both sides
				if (move.target === 'foeSide' || move.target === 'all') {
					this.attrLastMove('[still]');
					this.add('cant', source, 'ability: Prevent Moves', move, '[of] ' + target);
					return false; // Prevent the move for both teams
				}
			}
		},
		name: "Volcanic Ash",
