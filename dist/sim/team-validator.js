...
class TeamValidator {
  ...
  checkCanLearn(move, originalSpecies, setSources = this.allSources(originalSpecies), set = {}) {
    ...
    for (const { species, learnset } of fullLearnset) {
      ...
      let nextSpecies2 = dex.species.learnsetParent(baseSpecies);
      while (nextSpecies2) {
        if (nextSpecies2.name === species.name)
          break;
        nextSpecies2 = dex.species.learnsetParent(nextSpecies2);
      }
      if (checkingPrevo && !nextSpecies2)
      ...
      for (let learned of sources) {
        ...
        if (setSources.learnsetDomain && !setSources.learnsetDomain.includes(learnedGen + species.id) && (learned.charAt(1) !== "E" || learnedGen < 8)) {
          if (!cantLearnReason) {
        ...
        if (learnedGen >= 8 && learned.charAt(1) === "E" && learned.slice(1) !== "Eany" && learned.slice(1) !== "Epomeg" || "LMTR".includes(learned.charAt(1))) {
          if (learnedGen === dex.gen && learned.charAt(1) !== "R") {
            ...
            if (!moveSources.moveEvoCarryCount && !setSources.babyOnly)
              return null;
          }
    ...
    if (!tradebackEligible) {
      while (nextSpecies) {
        for (let gen = nextSpecies.gen; gen <= dex.gen; gen++) {
          if (canLearnSpecies.includes(nextSpecies.id) || 0 < speciesCount && speciesCount < canLearnSpecies.length || speciesCount === 0 && gen >= minLearnGen || speciesCount === canLearnSpecies.length && gen <= moveSources.sourcesBefore) {
